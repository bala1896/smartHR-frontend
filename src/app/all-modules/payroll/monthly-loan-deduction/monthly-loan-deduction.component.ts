...
declare const $: any;
@Component({
  selector: 'app-monthly-loan-deduction',
  templateUrl: './monthly-loan-deduction.component.html',
  styleUrls: ['./monthly-loan-deduction.component.css']
})
export class MonthlyLoanDeductionComponent implements OnInit, OnDestroy {
...
loandeductedlist()
{
  ...
  this.data.runQuery(sql).subscribe((data) => {
    console.log(data)
     this.loandeductedlist = data.map(item=>{return {
       uid:item.uid,
       tyear:item.tyear,
       tmonth:item.tmonth,
       notes:item.notes,
